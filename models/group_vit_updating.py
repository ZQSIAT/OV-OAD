        ...
        if self.use_enc_feat and self.i_layer == 0:
            if self.enc_feat_lsxattn:
                if self.long_term_detach:  # 33.7 up to 33.8  # TODO !!
                    ...
                    # long_term_feats, short_term_feats = x[:, :-self.short_term_len, :].clone().detach().contiguous(), x[:, -self.short_term_len:, :].clone().contiguous()
                else:
...
class GroupViT(nn.Module):
    ...
    def __init__(self,
        ...
        mask_flag = True if use_direc_mask else False
        for i_layer in range(self.num_layers):
            if i_layer > 0: # 只有layer 0 添加 direc mask
                mask_flag = False
