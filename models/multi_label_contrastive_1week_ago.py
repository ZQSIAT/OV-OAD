...
class MultiLabelContrastive(nn.Module):
    ...
    def forward_train(self, image, text, enc_saliency_caption=None, enc_targets=None, cross_image=None, cross_entity=None, \
        ...
        if self.use_saliency:
            ...
            saliency_loss = self.loss(saliency_feat, saliency_text) * self.saliency_weight
            losses_dict['saliency'] = saliency_loss
            # embed()
            # exit()
        ############################################################
        ...
        maskloss = .0
        if self.use_maskloss:   # default: False  
            ...
            maskloss = dice_loss(pred_mask, gt_mask,) 
            # maskloss = sigmoid_focal_loss(pred_mask, gt_mask,) 
        ...
        elif self.use_entityloss: 
            losses = matchingloss + self.entity_weight * entityloss
        elif self.use_maskloss: # 一般不单独使用mask loss
