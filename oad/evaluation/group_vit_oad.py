...
# for batch size = 1
class GroupViTOadInferenceBS1(nn.Module):
    # def __init__(self, model, text_embedding, with_bg, test_cfg=dict(mode='whole', bg_thresh=.95, use_clip=False)):
    def __init__(self, model, text_embedding, with_bg, test_cfg=dict(mode='whole', bg_thresh=.33)):  # TODO .33 > .95 ?
        super().__init__()
        ...
        logger = get_logger()
        logger.info(
            f'Building GroupViTOadInference with {self.num_classes} classes, test_cfg={test_cfg}, with_bg={with_bg}')
        # embed()
        # exit()
    ...
    def get_attn_maps(self, feats, return_onehot=False, rescale=False):
        """
        Args:
            feats: [B, L, C]
        ...
        """
        results = self.model.img_encoder(feats, return_attn=True, as_dict=True)
        ...
        with torch.no_grad():
            ...
            for idx, attn_dict in enumerate(results['attn_dicts']):
                ...
                attn_masks = attn_dict['soft']  # soft = hard
                # attn_masks = attn_dict['hard'] 
                ...
                # 取最后一个group token需要rescale sequence length
                # print(prev_attn_masks.shape, attn_masks.shape, '\n')
                # OAD return [B, nH, L, G], L= HxW
                attn_maps.append(prev_attn_masks)

        for i in range(len(attn_maps)):
            attn_map = attn_maps[i] # torch.Size([1, 1, 128, 8])
            # embed()
            # exit()
            # [B, nh, L, G]
            ...
            # [B, L, G]
            attn_map = attn_map.squeeze(1) # torch.Size([1, 128, 8])
            ...
            attn_maps[i] = attn_map  # [1, 128, 8]
        
        return attn_maps

    def forward(self, feats):
        """Encode frames feats with backbone and decode into a action segmentation
        map of the same size as input."""

        assert feats.shape[0] == 1, 'batch size must be 1'
        # B = feats.shape[0]

        # [B, L, G], get the last one only [256, 128, 8]
        attn_map = self.get_attn_maps(feats, rescale=False)[-1]
        # attn_map = self.get_attn_maps(feats, rescale=False)[0]
        # embed()
        # exit()

        # [B, L, G], select batch idx 0
        attn_map = attn_map[0] # [1, 128, 8] -> [128, 8]
        img_outs = self.model.encode_image(feats, return_feat=True, as_dict=True)

        # B: 256, G: 8, C: Dim 256
        grouped_img_tokens = img_outs['image_feat'].squeeze(0)
        img_avg_feat = img_outs['image_x'] # [256, 256]    

        # normalize [B, G, C] [256, 8, 256]
        grouped_img_tokens = F.normalize(grouped_img_tokens, dim=-1)
        img_avg_feat = F.normalize(img_avg_feat, dim=-1) # [256, 256]

        # [B L G] [256, 128, 8]
        onehot_attn_map = F.one_hot(attn_map.argmax(dim=-1), num_classes=attn_map.shape[-1]).to(dtype=attn_map.dtype)
        num_fg_classes = self.text_embedding.shape[0] # 20
        class_offset = 1 if self.with_bg else 0
        text_tokens = self.text_embedding # [20, 256]
        num_classes = num_fg_classes + class_offset # 21
        logit_scale = torch.clamp(self.model.logit_scale.exp(), max=100)

        # [B, G, num_classes] = [256, 8, 20]
        group_affinity_mat = (grouped_img_tokens @ text_tokens.T) * logit_scale
        pre_group_affinity_mat = F.softmax(group_affinity_mat, dim=-1) # [256, 8, 20]

        # 所有帧的全局描述
        avg_affinity_mat = (img_avg_feat @ text_tokens.T) * logit_scale # [256, 20]
        avg_affinity_mat = F.softmax(avg_affinity_mat, dim=-1) # [256, 20]
        
        affinity_mask = torch.zeros_like(avg_affinity_mat) # [256, 20]
        avg_affinity_topk = avg_affinity_mat.topk(dim=-1, k=min(5, num_fg_classes)) # TODO: topk为什么是15 22.83
        affinity_mask.scatter_add_(dim=-1, index=avg_affinity_topk.indices, src=torch.ones_like(avg_affinity_topk.values))  # [256, 20]
        # embed()
        # exit()
        group_affinity_mat.masked_fill_(~affinity_mask.bool(), float('-inf'))
        # group_affinity_mat.permute(1, 0, 2).masked_fill_(~affinity_mask.bool(), float('-inf')) # [8, 256, 20].masked_fill_([256, 20], -inf) -> [256. 8, 20]
        group_affinity_mat = F.softmax(group_affinity_mat, dim=-1) # [256, 8, 20]

        # TODO: check if necessary
        group_affinity_mat *= pre_group_affinity_mat # [256, 8, 20] 逐元素相乘 TODO 注释掉更好why?
        pred_logits = torch.zeros(num_classes, attn_map.shape[0], device=feats.device, dtype=feats.dtype)
        # pred_logits = torch.zeros(B, num_classes, attn_map.shape[1], device=feats.device, dtype=feats.dtype) # [256, 21, 128]
        # pred_logits[:, class_offset:, :] = rearrange(onehot_attn_map @ group_affinity_mat, 'b l c -> b c l') # pred_logits[1:] = [256, 20, 128]

        pred_logits[class_offset:] = rearrange(onehot_attn_map @ group_affinity_mat, 'l c -> c l') # pred_logits[1:] = [256, 20, 128]
        ...
        # if self.with_bg:
        #     bg_thresh = torch.full((B,), self.bg_thresh, device=feats.device)
        #     mask_min = torch.lt(bg_thresh, group_affinity_mat.max(1).values.max(1).values)
        #     bg_thresh = torch.where(mask_min, bg_thresh, group_affinity_mat.max(1).values.max(1).values)
        #     pred_logits.permute(1, 0, 2)[0, (onehot_attn_map @ group_affinity_mat).max(dim=-1).values < bg_thresh.unsqueeze(1)]=1

        ## output action onehot prediction
        outputs = F.softmax(pred_logits.unsqueeze(0), dim=1).permute(0, 2, 1).contiguous()  # .argmax(dim=1) 
        return outputs # B L C -> [1, 24, 21]
...
# for batch size > 1
class GroupViTOadInference(nn.Module):

    # def __init__(self, model, text_embedding, with_bg, test_cfg=dict(mode='whole', bg_thresh=.95, use_clip=False)):
    def __init__(self, model, text_embedding, saliency_text_embedding, config, with_bg, test_cfg=dict(mode='whole', bg_thresh=.33)):  # TODO .33 > .95 ?
        super().__init__()
        if not isinstance(test_cfg, mmcv.Config):
            test_cfg = mmcv.Config(test_cfg)
        self.test_cfg = test_cfg
        ...
        # [N, C]
        self.register_buffer('text_embedding', text_embedding)
        self.register_buffer('saliency_text_embedding', saliency_text_embedding)
    ...
    def forward(self, feats):
        ...
        # B: 256, G: 8, C: Dim 256
        grouped_img_tokens = img_outs['image_feat']
        img_avg_feat = img_outs['image_x'] # [256, 256]    
        ...
        ## output action onehot prediction
        outputs = F.softmax(pred_logits, dim=1).permute(0, 2, 1).contiguous()  # .argmax(dim=1) 

        saliency_logits = None
        if self.config.use_saliency:
            saliency_logits = self.model.zero_shot_saliency(feats, self.saliency_text_embedding)
        
        enc_feat_logits = None
        if self.config.use_enc_feat:
            enc_feats = img_outs['enc_feats'][0]
            # embed()
            # exit()
            enc_feat_logits = self.model.zero_shot_enc_feat(enc_feats, self.saliency_text_embedding)


        return {"temporal": outputs, "saliency": saliency_logits, "enc_feat_logits": enc_feat_logits} # B L C -> [256, 128, 21], B C -> [256, 21]
